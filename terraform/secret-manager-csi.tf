resource "helm_release" "secrets_csi_driver" { # driver onde responsavel por armazenar como volume as secrets no containers
...
data "aws_iam_policy_document" "myapp_secrets" {
  statement {
    ...
    condition {
      ...
      values   = ["system:serviceaccount:example:myapp"]
    }
